...
void compare_prof_W_inWindow_ptpair_improved() {
    setTDRStyle();

    const TString histName = "prof_W_inWindow_ptpair_improved";
    ...
    TH1D *h = tdrHist("h", "Mean W mass", 0, 160, "p_{T,pair}", 0, 250);
    TCanvas* c1 = tdrCanvas("c1", h, 2024, 0, kSquare);
    hists[0]->GetXaxis()->SetTitle("p_{T,pair} (GeV)");
    hists[0]->GetYaxis()->SetTitle("Mean W mass (GeV)");
    hists[0]->GetYaxis()->SetRangeUser(70, 95);
    ...
    c1->SaveAs("prof_W_inWindow_ptpair_improved_comparison.pdf");
